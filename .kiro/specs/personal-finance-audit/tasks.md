...
Implement a FastAPI backend with SQLAlchemy/SQLite persistence, bank-specific CSV/PDF adapters, rule-based categorization with GPT-4o-mini fallback, anomaly detection, and a Streamlit frontend. Property-based tests use Hypothesis for all 17 correctness properties.
...
- [ ] 2. Database models and schema
  - [ ] 2.1 Define SQLAlchemy ORM models in `app/models/db.py`
  ...
  - [ ] 2.2 Define Pydantic schemas in `app/models/schemas.py`
  ...
  - [ ]* 2.3 Write property test for Transaction serialization round-trip
  ...
  - [ ]* 2.4 Write property test for malformed payload → HTTP 422
...
- [ ] 3. ID derivation and Duplicate_Filter
  - [ ] 3.1 Implement `derive_id()` in `app/services/duplicate_filter.py`
  ...
  - [ ]* 3.2 Write property test for ID determinism
  ...
  - [ ]* 3.3 Write property test for deduplication idempotency
  ...
  - [ ]* 3.4 Write property test for batch summary consistency
...
- [ ] 4. PDF_Parser
  - [ ] 4.1 Implement `PDF_Parser` in `app/services/pdf_parser.py`
  ...
  - [ ]* 4.2 Write property test for PDF full-page coverage
  ...
  - [ ]* 4.3 Write unit tests for PDF_Parser edge cases
...
- [ ] 5. Bank_Adapter base class and concrete implementations
  - [ ] 5.1 Implement `BankAdapter` ABC and `AdapterRegistry` in `app/adapters/base.py`
  ...
  - [ ] 5.2 Implement `HDFCSavingsAdapter` in `app/adapters/hdfc.py`
  ...
  - [ ] 5.3 Implement `ICICISavingsAdapter` in `app/adapters/icici.py`
    - Define `header_signature` matching ICICI savings CSV columns
    - Implement `parse_csv_rows()` and `parse_pdf_text()` for ICICI format
    - _Requirements: 1.2, 1.4, 1.9_

  - [ ] 5.4 Implement `GenericCreditCardAdapter` in `app/adapters/credit_card.py`
    - Define `header_signature` for generic credit card CSV format
    - Implement `parse_csv_rows()` and `parse_pdf_text()`
    - Register all three adapters in `app/adapters/__init__.py` via `AdapterRegistry`
    - _Requirements: 1.2, 1.4, 1.9_

  - [ ]* 5.5 Write property test for adapter selection exhaustiveness
  ...
  - [ ]* 5.6 Write property test for adapter field mapping
  ...
  - [ ]* 5.7 Write unit tests for Bank_Adapters with fixture files
    - One example-based test per adapter using a sample CSV fixture in `tests/fixtures/`
    - _Requirements: 1.2, 1.4_

- [ ] 6. Categorization_Engine and Rule_Store
  - [ ] 6.1 Implement `CategorizationEngine` in `app/services/categorization.py`
  ...
  - [ ] 6.2 Implement rule CRUD helpers in `app/services/rule_service.py`
  ...
  - [ ]* 6.3 Write property test for highest-priority rule wins
  ...
  - [ ]* 6.4 Write property test for LLM fallback invocation
...
- [ ] 7. Anomaly_Detector
  - [ ] 7.1 Implement `AnomalyDetector` in `app/services/anomaly_detector.py`
  ...
  - [ ]* 7.2 Write property test for anomaly threshold exactness
  ...
  - [ ]* 7.3 Write property test for deviation percentage correctness
  ...
  - [ ]* 7.4 Write property test for bucket mapping totality
  ...
  - [ ]* 7.5 Write property test for bucket totals correctness
...
- [ ] 8. FastAPI routes
  - [ ] 8.1 Implement `POST /api/v1/upload` in `app/routes/upload.py`
  ...
  - [ ]* 8.2 Write property test for multi-file merge
  ...
  - [ ] 8.3 Implement `GET /api/v1/transactions` in `app/routes/transactions.py`
    - Query all transactions from DB; serialize using `Transaction` schema
    - _Requirements: 7.1, 7.2_

  - [ ] 8.4 Implement `PATCH /api/v1/transactions/:id` in `app/routes/transactions.py`
  ...
  - [ ]* 8.5 Write property test for correction updates fields
  ...
  - [ ]* 8.6 Write property test for correction persists rule
  ...
  - [ ] 8.7 Implement `GET /api/v1/anomalies` and `GET /api/v1/summary` in `app/routes/analytics.py`
  ...
  - [ ] 8.8 Implement `GET/POST/DELETE /api/v1/rules` in `app/routes/rules.py`
    - Wire to `rule_service` CRUD helpers
    - _Requirements: 4.4_

  - [ ] 8.9 Wire all routers in `app/main.py`
...
- [ ] 11. Integration and smoke tests
  - [ ] 11.1 Write integration test for full upload → categorize → retrieve flow
  ...
  - [ ]* 11.2 Write integration tests for correction flow
  ...
  - [ ]* 11.3 Write integration tests for anomaly detection flow
  ...
  - [ ]* 11.4 Write smoke tests in `tests/smoke/test_smoke.py`
    - Assert all 3 adapters are registered in AdapterRegistry
