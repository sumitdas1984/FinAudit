...
class AxisSavingsAdapter(BankAdapter):
    @property
    def header_signature(self) -> frozenset[str]:
        return frozenset({"Tran Date", "PARTICULARS", "DR", "CR", "BAL"})

    @property
    def text_patterns(self) -> list[str]:
        return ["Axis Bank", "AXIS BANK LIMITED"]

    def parse_csv_rows(self, reader: csv.DictReader) -> list[RawTransaction]:
        transactions = []
        for row in reader:
            debit_raw = row.get("DR", "").strip().replace(",", "")
            credit_raw = row.get("CR", "").strip().replace(",", "")
            ...
            date_str = row["Tran Date"].strip()
            parsed_date = _parse_date(date_str)
            description = row["PARTICULARS"].strip()
...
AdapterRegistry.register(AxisSavingsAdapter())
